            ...
            if "all_data_submit" in request.POST:
                ...
                for i in range(len(restype)):
                    ...
                    if restype[i] == "btn":
                        ...
                        res_data += f"""<div class=\"row justify-content-start form-input-wrap\"><div class=\"col\"><button type=\"button\"  id=\"dynamicbtn\" onClick=\"GFG_click(this.value)\" value=\"{btnlabel[i]}\" class=\"btn form-input\">{btnlabel[i]}</button></div></div>"""

                    if restype[i] == "lbl":
                        print("lbllabel: ",btnlabel[i])
                        print("lblname: ",btnname[i])
                        print("lbltheme: ",btntheme[i])
                        res_data += f"""<div class=\"row justify-content-start form-input-wrap\"><div class=\"col-lg-12 col-sm-10\"><p class=\"bot-chat botside-message\" id=\"bot-msg\">{btnlabel[i]}</p></div></div>"""
                    
                    if restype[i] == "lnk":
                        print("lnklabel: ",btnlabel[i])
                        print("lnkname: ",btnname[i])
                        print("lnktheme: ",btntheme[i])
                        # res_data += f"""<div class=\"row justify-content-start form-input-wrap\"> <div class=\"col-lg-12 col-sm-10\"><p class=\"bot-chat botside-message\" id=\"bot-msg\"><a target=\"_blank\" href=\"{btnlabel[i]}\">{btnname[i]}</a></p></div></div>"""
                        res_data += f"""<div class=\"row justify-content-start form-input-wrap\"><div class=\"col-lg-12 col-sm-10\"><label class=\"form-check-label\" for=\"flexCheckDefault\"><a target=\"_blank\" href=\"{btnlabel[i]}\" class=\"chat-link\">{btnname[i]}</a></label> </div></div>"""
...
def advance_bot_builder(request):
    if "email" not in request.session and "type" not in request.session and "id" not in request.session:
        messages.error(request, "Login first")
        return redirect("sign-in")
    if request.session["type"] != "client":
    ...
    if request.method == "POST":
        try:
            ...
            if "all_data_submit" in request.POST:
                ...
                print("_chatbotregister.pk:",_chatbotregister.pk)
                _chatbotregister.url_link = f"http://{urlObject}/client-app/chat-bot-{generator.encrypt(_register_data.pk)}-{generator.encrypt(_chatbotregister.pk)}/"
                _chatbotregister.save()
                ...
                try:
                    intents = json.loads(request.POST.get("all_data"))
                except Exception:
                    intents = {}
            ...
            return redirect("appclient:advance_bot_builder")

    context = {"footer": {"url": "https://colorlib.com", "name": "cloud4code", "year": datetime.now().year, "extra_name": "chatbot"}, "title": "Client-build-bot"}
    return render(request,"client_temp/advance_builder.html",context)
...
def clientdeletechatbot(request,pk):
    ...
    if os.path.exists(f'./clientdb/{_delete_data.client_id.fname}_{_delete_data.client_id.pk}/{_delete_data.chatbot_name}-{_delete_data.pk}_words.pkl'):
        os.remove(f'./clientdb/{_delete_data.client_id.fname}_{_delete_data.client_id.pk}/{_delete_data.chatbot_name}-{_delete_data.pk}_words.pkl')
    else:
        print("The file does not exist")

    if os.path.exists(f'./clientdb/{_delete_data.client_id.fname}_{_delete_data.client_id.pk}/{_delete_data.chatbot_name}-{_delete_data.pk}_classes.pkl'):
        os.remove(f'./clientdb/{_delete_data.client_id.fname}_{_delete_data.client_id.pk}/{_delete_data.chatbot_name}-{_delete_data.pk}_classes.pkl')
    else:
        print("The file does not exist")

    
    if os.path.exists(f"./clientdb/{_delete_data.client_id.fname}_{_delete_data.client_id.pk}/{_delete_data.chatbot_name}-{_delete_data.pk}_{_delete_data.category}.json"):
        os.remove(f"./clientdb/{_delete_data.client_id.fname}_{_delete_data.client_id.pk}/{_delete_data.chatbot_name}-{_delete_data.pk}_{_delete_data.category}.json")
    else:
        print("The file does not exist")

    if os.path.exists(f'./clientdb/{_delete_data.client_id.fname}_{_delete_data.client_id.pk}/{_delete_data.chatbot_name}-{_delete_data.pk}_chatbot_model.h5'):
        os.remove(f'./clientdb/{_delete_data.client_id.fname}_{_delete_data.client_id.pk}/{_delete_data.chatbot_name}-{_delete_data.pk}_chatbot_model.h5')
    else:
        print("The file does not exist")
